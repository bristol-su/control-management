<template>
    <div>
        <the-title title="Create a Position Tag Category"></the-title>
        ...
        methods: {
            create() {
                ...
                this.$control.positionTagCategory().create(this.form)
                    .then(response => this.$router.push({name: 'positionTagCategory', params: {'positionTagCategoryId': response.data.id}}))
                    .catch(() => {})
