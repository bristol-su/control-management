<template>
    <div>
        <the-title title="Create a Group Tag"></the-title>
...
<script>
    ...
    import VGroupTagCategoryDropdown from "../../components/common/VGroupTagCategoryDropdown";
    import {mapActions} from "vuex";

    export default {
        name: "GroupTagCreate",
        components: {VGroupTagCategoryDropdown, VButtonSubmit, TheTitle},
        data() {
            return {
                form: {
                    name: null,
        ...
        methods: {
            create() {
                ...
                this.$control.groupTag().create(this.form)
                    .then(response => this.$router.push({name: 'groupTag', params: {'groupTagId': response.data.id}}))
                    .catch(() => {})
