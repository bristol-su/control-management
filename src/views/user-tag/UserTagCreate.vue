<template>
    <div>
        <the-title title="Create a User Tag"></the-title>
...
<script>
    ...
    import VUserTagCategoryDropdown from "../../components/common/VUserTagCategoryDropdown";
    import {mapActions} from "vuex";

    export default {
        name: "UserTagCreate",
        components: {VUserTagCategoryDropdown, VButtonSubmit, TheTitle},
        data() {
            return {
                form: {
                    name: null,
        ...
        methods: {
            create() {
                ...
                this.$control.userTag().create(this.form)
                    .then(response => this.$router.push({name: 'userTag', params: {'userTagId': response.data.id}}))
                    .catch(() => {})
