    ...
    <div>
        ...
        <b-table :items="tags" :fields="fields" :busy="loading" small v-else>
            ...
            <template v-slot:cell(actions)="data">
                <v-button-view
                          @view="$router.push({name: 'userTag', params: {userTagId: data.item.id}})">
                </v-button-view>
                <v-button-delete
                        @delete="deleteTag(data.item.id)"
                        :delete-text="'Delete tag ' + data.item.name + ' from user?'"></v-button-delete>
            </template>
        </b-table>
        <v-pagination
                :per-page="perPage"
...
<script>
    ...
    export default {
        ...
        methods: {
            deleteTag(tagId){
                this.$control.user().untag(this.userId, tagId)
                    .then(() => {
                        Notify.success('Tag removed from user')
                        this.fetchTags();
                    });
            },
            fetchTags() {
